...
@section('title', 'Guest Chat Support')
   ...
   <section class="content">
      <div class="card">
         <div class="card-header">
            <h3 class="card-title"><i class="fas fa-comments"></i> Guest Chat Support</h3>
         </div>
         <!-- /.card-header -->
         <div class="card">
            <div class="row">
                  ...
                  <div class="card-body">
                     <ul class="chat_user_ul">
                       ...
                       <li onclick="switchUser({{$suser->id}})" id="chat_user_{{ $suser->id }}"  class="{{ ($suser->id == $user->id)?"active_chat":"" }} chat_user_li">
                         {!! $suser->username ."<p><small>(" . $suser->session_id .")</small></p>" !!}
                         @php
...
function sendMessage(event)
 {
   ...
   var formData = new FormData(document.getElementById("form_send_message"));
      ...
      $.ajax({
                  url: "{{ route('admin.guest.chat.message.store') }}",
                  type: 'post',
                  data: formData,
                 async: false,
                 cache: false,
...
function update_last_activity()
 {
   ...
   $.ajax({
                type: "GET",
                url: "<?php echo url("admin/guest_chat_support/api/get-unread/".Auth::user()->id); ?>/"+$('#receiver_id').val(),
                data: '',
                  ...
                  if(data.sorted_user.length != 0){
                    ...
                    $.each( data.sorted_user, function( key, value ) {
                      if( value.id == $('#receiver_id').val() ){
                        ...
                        active_html += value.username +'<p><small>('+ value.session_id +')</small></p>';
                        active_html += '</li>';
                      }else {
                        var class_name = '';
                        html += '<li onclick="switchUser('+value.id+')" id="chat_user_'+value.id+'"  class="'+class_name+' chat_user_li">';
                        html += value.username +'<p><small>('+ value.session_id +')</small></p>';
                    ...
                    $('.chat_user_ul').html(active_html + html);
                  }
...
function switchUser(user_id){
  ...
  $.ajax({
               type: "GET",
               url: "<?php echo url('admin/guest_chat_support/api/get-chat/'.Auth::user()->id); ?>/" + $('#receiver_id').val(),
               data: '',
               success: function (data) {
                 if(data != 0){
                   html = '';
